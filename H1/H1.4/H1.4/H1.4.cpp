// H1.4.cpp : Defines the entry point for the application.
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		SetTimer(hWnd, 333, 1000, TIMERPROC(NULL));
	}
	break;
	case WM_TIMER:
	{
	...
	case WM_PAINT:
	{
		...
		char g[111];
		WORD a;
		sprintf_s(g, "%d:%d:%d", st.wHour, st.wMinute, st.wSecond);
		if (st.wSecond < 10)
		{
			int a = 4;
		}
		OffsetRect(t, (client->right - t->right) / 2, (client->bottom - t->bottom) / 2);
		std::wstring tmp = std::wstring(g, g + strlen(g));
		DrawText(hdc, tmp.c_str(), tmp.length()+1, t, DT_TOP);
		EndPaint(hWnd, &ps);
	}
